...
# 2. Install PHP extensions (GD with JPEG/FreeType support, MySQLi, Curl)
...
# 4. Silent mode: Hide PHP warnings and notices for legacy compatibility
...
# --- FIX 1: Disable UTF8 check in installer (since we handle it via MySQL config) ---
...
# --- FIX 2: Plug & Play Permissions (Pre-create config and set owner) ---
# We create the file first, then give ownership to the web server user (www-data)
